...
@Component({
  selector: 'app-root',
  standalone: true,
  imports: [CommonModule, FormsModule, UniTableComponent, UniTemplateDirective], // Add FormsModule here
  templateUrl: './app.component.html',
  styleUrl: './app.component.scss'
})
export class AppComponent {
  title = 'uni-table-demo';
  searchTerm: string = ''; // New property for manual search

  tableConfig: UniTableConfig = { // Renamed from dtOptions
    paging: true,
    searching: true,
    colVis: true,
    pageLength: 5,
    overflow: 'responsive', // or 'scroll'
    storageKey: 'uni_table_demo_state', // New
    autoSaveState: true, // New
    showSaveControls: true, // New
    manualSearch: true, // Enable manual search
    showContextMenu: true, // Enable context menu
  };

  externalFilters: any = { globalSearch: '' }; // New property for external filters

  dataConfig: UniDataConfig = {
    columns: [
      { key: 'id', title: 'ID', width: '50px', priority: 10 },
      { key: 'name', title: 'Name', priority: 5, },
      { key: 'position', title: 'Position of the great person in the office',  priority: 4, headerWrap: true },
  ...
  get salaryCellStyle() {
    return (row: any, col: UniColumn) => {
      ...
      return { 'background-color': '', 'color': '' }; // Return empty strings for properties
    };
  }

  restoreFilters(filters: any) {
    console.log('Restoring filters:', filters);
    if (filters) {
      this.externalFilters = { ...filters };
    } else {
      this.externalFilters = { globalSearch: '' };
    }
    this.searchTerm = this.externalFilters.globalSearch || ''; // Update searchTerm from restored filters
  }

  onSearchTermChange(term: string) {
    this.searchTerm = term;
    this.externalFilters = { globalSearch: term };
  }
